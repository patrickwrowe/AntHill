...
class MetaMap(maps.MapArray):
    ...
    @classmethod
    def new_map(
        cls, sub_maps: List[maps.MapArray], coefficients: List[float]
    ) -> MetaMap:
    ...
    @staticmethod
    def validate_submaps(
        sub_maps: List[maps.MapArray], coefficients: List[float]
    ) -> None:
    ...
    def recompose_submaps(self):
        self.values = MetaMap._compose_submaps(
            sub_maps=self.sub_maps, coefficients=self.coefficients
        )
