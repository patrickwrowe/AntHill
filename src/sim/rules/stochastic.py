from src.sim.datatypes import entities, maps, SimPos
...
def metropolis_monte_carlo(sim_entites: List[entities.Entity], potential: maps.MapArray):
    """
    ...
    for entity in sim_entites:
        ...
        for i in range(sconf.mmc_max_attempts):
            move, accepted = metropolis_move(potential=potential,
                                                pos=entity.pos)
            if accepted:
                entity.pos.update_pos(move)

def metropolis_move(potential: maps.MapArray, pos: SimPos, temperature: float = sconf.mmc_move_temp, move_size=sconf.mmc_move_size) -> Tuple[Tuple[float, float], bool]:
    ...
    delta_energy = potential.normalised_values[int(pos_new.y), int(pos_new.x)] - potential.normalised_values[int(pos.y), int(pos.x)]
