...
def test_metropolis_move():
    ...
    test_potential = MapArray(np.array([[1, 1, 1], [1, 2, 1], [1, 1, 1]]))
    ...
    test_pos = SimPos(1, 1)
    move, accepted = metropolis_move(
        test_potential, test_pos, temperature=1.0, move_size=1
    )
    assert np.all(np.array(move) < 1)
    assert accepted == True
    ...
    # Create a test potential
    test_potential = MapArray(
        2 * np.ones((sconf.default_map_resolution_x, sconf.default_map_resolution_y))
    )
    ...
    test_pos = SimPos(center_x, center_y)
    move, accepted = metropolis_move(
        test_potential, test_pos, temperature=10, move_size=1
    )
    assert np.all(np.array(move) < 1)
    assert accepted == True
    ...
    move, accepted = metropolis_move(
        ...
        test_pos,
        temperature=1,
        move_size=sconf.default_map_resolution_x * 100,
    )
